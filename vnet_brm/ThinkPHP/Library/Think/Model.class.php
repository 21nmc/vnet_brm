...
class Model {
    ...
    /**
     * 创建数据对象 但不保存到数据库  by zwj
     * @access public
     * @param mixed $hrm_data 创建数据
     * @param string $type 状态
     * @return mixed
     */
     public function create_hrm($hrm_data='',$type='') {
        // 如果没有传值默认取POST数据
        if(empty($hrm_data)) {
            $hrm_data   =   I('post.');
        }elseif(is_object($hrm_data)){
            $hrm_data   =   get_object_vars($hrm_data);
        }
        // 验证数据
        if(empty($hrm_data) || !is_array($hrm_data)) {
            $this->error = L('_DATA_TYPE_INVALID_');
            return false;
        }

        // 状态
        $type = $type?:(!empty($hrm_data[$this->getPk()])?self::MODEL_UPDATE:self::MODEL_INSERT);

        // 检查字段映射
        $hrm_data = $this->parseFieldsMap($hrm_data,0);
        // 检测提交字段的合法性
        if(isset($this->options['field'])) { // $this->field('field1,field2...')->create()
            $fields =   $this->options['field'];
            unset($this->options['field']);
        ...
        // 数据自动验证
        // if(!$this->autoValidation($hrm_data,$type)) return false;

        // // 表单令牌验证
        // if(!$this->autoCheckToken($hrm_data)) {
        //     $this->error = L('_TOKEN_ERROR_');
        //     return false;
        // }
        ...
        if($this->autoCheckFields) { // 开启字段检测 则过滤非法字段数据
            ...
            foreach ($hrm_data as $key=>$val){
                if(!in_array($key,$fields)) {
                    ...
                    $hrm_data[$key] =   stripslashes($val);
                }
            }
        }

        // 创建完成对数据进行自动处理
        $this->autoOperation($hrm_data,$type);
        // 赋值当前数据对象
        $this->hrm_data =   $hrm_data;
        // 返回创建的数据以供其他调用
        return $hrm_data;
     }
