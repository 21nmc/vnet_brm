...
//var_dump($row_pro = mysql_fetch_array($result_pro));die;
...
while ($row_agent = mysql_fetch_array($result_agent))
{
  ...
  if (empty($check_agentid)){
      $sql2_agent="insert into vnet_agents (title,type)values('".$agentname."',3)";
      //echo $sql2_agent."----<br />";
...
//--------------------------------------------------------------------------------------
  ...
  $sql_old_rack_name = "select name from Object where id=".$rack_id." ";
  $result_rack_name = mysql_query($sql_old_rack_name,$dbconn_76);
  if ($row_rack_name_now = mysql_fetch_array($result_rack_name))
  {
      $temp_rack_name =  $row_rack_name_now['name'];
      $sql_rack_final="select id from vnet_ipdb_racks where name='".$temp_rack_name."'"; //现在rackid
      $result_rack_final = mysql_query($sql_rack_final,$con);
      if ($rack_id_info = mysql_fetch_array($result_rack_final))
      {
          $rack_id =  $rack_id_info['id'];
      }
  }
