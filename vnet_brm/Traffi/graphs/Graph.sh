...
PERIOD=3600
WIDTH=800
GRAPH_DIR="/var/www/html/Traffi/graphs"
COOKIE="/tmp/zabbix_cookie"
CURL="/usr/bin/curl"
INDEX_URL="$ZBX_URL/index.php"
CHART_URL="$ZBX_URL/chart2.php"
...
while getopts U:P:I:s:p:w:h OPTION;do
        case $OPTION in
                ...
                #check_integer "$PERIOD"
                 #  || { echo "ERROR: Field 'PERIOD' is not integer."; exit 1;}
...
#PNG_PATH="$GRAPH_DIR/$ITEMID.$TIME.png"
PNG_PATH="$GRAPH_DIR/$ITEMID.png"
# 获取item的graph，保存为png图片
${CURL} -b ${COOKIE} -d "graphid=${ITEMID}&period=${PERIOD}&stime=${STIME}" $CHART_URL > "$PNG_PATH"
#[ -s "$PNG_PATH" ] && echo "Saved the graph as $PNG_PATH" || echo "Failed to get the graph."
[ -s "$PNG_PATH" ]  || echo "Failed to get the graph."
